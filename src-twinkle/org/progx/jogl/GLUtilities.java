...
public class GLUtilities {
    public static void drawLocalAxis(GL gl, float axisLength) {
        ...
        gl.getGL2().glColor3f(1, 0, 0);
        gl.getGL2().glVertex3f(0, 0, 0);
        gl.getGL2().glVertex3f(x.x * axisLength, x.y * axisLength, x.z * axisLength);

        gl.getGL2().glColor3f(0, 1, 0);
        gl.getGL2().glVertex3f(0, 0, 0);
        gl.getGL2().glVertex3f(y.x * axisLength, y.y * axisLength, y.z * axisLength);

        gl.getGL2().glColor3f(0, 0, 1);
        gl.getGL2().glVertex3f(0, 0, 0);
        gl.getGL2().glVertex3f(z.x * axisLength, z.y * axisLength, z.z * axisLength);
        gl.getGL2().glEnd();
    ...
    public static void renderBillboard(GL gl,  Vector3f camPos, Vector3f camUp,
        ...
        gl.getGL2().glMultMatrixf(new float[] { right.x, right.y, right.z, 0.0f,
                                       up.x, up.y, up.z, 0.0f,
    ...
    public static void getCameraVectors(GL gl, Vector3f camPos, Vector3f camUp) {
        ...
        gl.getGL2().glGetFloatv(GL2.GL_MODELVIEW_MATRIX, matrix, 0);
    ...
    public static void renderAntiAliased(GL gl, Renderable scene) {
        ...
        gl.getGL2().glHint(GL2.GL_PERSPECTIVE_CORRECTION_HINT, GL.GL_NICEST);
        gl.getGL2().glHint(GL2.GL_POLYGON_SMOOTH_HINT, GL.GL_NICEST);
        gl.getGL2().glDisable(GL2.GL_DEPTH);
    ...
    public static void setFrustum(GL gl,
        ...
        gl.getGL2().glFrustum(left + dx, right + dx, bottom + dy, top + dy, near, far);
        gl.getGL2().glMatrixMode(gl.getGL2().GL_MODELVIEW);
        gl.getGL2().glLoadIdentity();
        gl.getGL2().glTranslatef((float) -eyedx, (float) -eyedy, 0.0f);
    }
